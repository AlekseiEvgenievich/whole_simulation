void macros_Diff_Theta_Phi_Uniform(){
//Note: This scrip is to set up the macro file for different angle(theta and phi)
	...
	for(int i_theta=0; i_theta<=90; i_theta+=5){
		...
		char cc[50];
		sprintf(cc, "Angle%i_%i_E%i_%i", i_theta, i_phi, int(min), int(max) );
		...
		//Pow Source
		output<<"/gps/ene/type Lin"<<endl;
		output<<"/gps/ene/min "<<min<<" keV"<<endl;
		output<<"/gps/ene/max "<<max<<" keV"<<endl;
		output<<"/gps/ene/gradient  0."<<endl;
		output<<"/gps/ene/intercept 1."<<endl;
