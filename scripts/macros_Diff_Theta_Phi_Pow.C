void macros_Diff_Theta_Phi_Pow(){
//Note: This scrip is to set up the macro file for different angle(theta and phi)
	...
	double max = 50000;
	//string path = "../macros/";
	//string path = "../macros/CNAF/gamma_theta_phi_pow_test/";
	string path = "../macros/alexei/";

	int i_phi = 0;
	for(int i_phi=0; i_phi<360; i_phi+=5)
		...
		char cc[50];
		sprintf(cc, "Angle%i_%i_Energy%i_%i", i_theta, i_phi, int(min), int(max) );
